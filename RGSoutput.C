...
struct Variables
{
	...
	//6 leading jets PT
	int jet1PT = addVariable("jet1PT");
	int jet2PT = addVariable("jet2PT");
	int jet3PT = addVariable("jet3PT");
	int jet4PT = addVariable("jet4PT");
	int jet5PT = addVariable("jet5PT");
	int jet6PT = addVariable("jet6PT");
...
TNtuple *AnalyseEvents(ExRootTreeReader *treeReader)
{
	...
	for(entry = 0; entry < ijloop; ++entry)
	{
		...
		//TODO fix, dont just have 2000
		float values[2000/*variables.names.size()*/] = { 0 };//All are initialized to zero, so anything not changed is still zero
		...
		for(int i=0;i<njet;i++) {//Loop over jets
			...
			switch (i){
				case 0:
					values[variables.jet1PT] = jet->PT;
					break;
				case 1:
					values[variables.jet2PT] = jet->PT;
					break;
				case 2:
					values[variables.jet3PT] = jet->PT;
					break;
				case 3:
					values[variables.jet4PT] = jet->PT;
					break;
				case 4:
					values[variables.jet5PT] = jet->PT;
					break;
				case 5:
					values[variables.jet6PT] = jet->PT;
					break;
...
void RGSoutput(const char *inputFile)
{
